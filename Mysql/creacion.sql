...
use tecnostore_db;
create table persona(id int not null auto_increment,nombre varchar (50) not null,identificacion varchar(50) unique not null,correo varchar(50)not null,telefono varchar(50),primary key(id));
create table marca(id int not null auto_increment,nombre_marca varchar(50),primary key(id));
create table modelo(id int not null auto_increment ,nombre_modelo varchar(50), marca_id int not null,primary key(id),foreign key(marca_id) references marca(id));
create table celular(id int not null auto_increment, unique modelo_id int not null
,sistema_operativo enum('Android','iOS','WindowsPhone','BlackBerry','HarmonyOS')
,gama enum('alta','media','baja'),precio double not null
,stock int default 0 not null, primary key(id),foreign key(modelo_id) references modelo(id));

create table cliente(id int not null,primary key(id), foreign key(id)references persona(id));
